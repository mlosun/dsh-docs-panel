window.__ModuleLoader__.load({ id: 'dsh-docs-panel', factory: (require) => {
  ...
  // React 通过模块加载器注入提供，不是浏览器全局变量（这是此前按钮不出现的根因之一）
  ...
  function apply(ctx) {
    ...
    injectStyles([
      // 按钮挂在右上角会话头部工具区（conversation.session.header.utilities），
      // 与「解读」等按钮同排的小按钮风格，规整、不参与侧边栏布局
      '.dsp-hdr-btn{display:inline-flex;align-items:center;gap:6px;cursor:pointer;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);border-radius:6px;padding:2px 10px;font-size:12px;font-family:inherit;line-height:1.5;white-space:nowrap}',
      '.dsp-hdr-btn:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dsp-hdr-btn-active{color:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary)}',
      '.dsp-backdrop{position:fixed;inset:0;z-index:39;background:rgba(10,12,18,0.4);pointer-events:auto}',
      '.dsp-panel{position:fixed;top:0;right:0;bottom:0;width:min(760px,94vw);z-index:40;border-left:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);box-shadow:-16px 0 48px rgba(0,0,0,0.22);flex-direction:column;display:flex;pointer-events:auto}',
      '.dsp-resizer{position:absolute;left:-5px;top:0;bottom:0;width:11px;cursor:col-resize;z-index:6;touch-action:none}',
      '.dsp-resizer:hover,.dsp-resizer.dsp-resizing{background:var(--dsw-alias-brand-primary);opacity:0.35}',
      '.dsp-header{box-sizing:border-box;border-bottom:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);flex:none;justify-content:space-between;align-items:center;min-height:48px;padding:8px 14px;display:flex;gap:8px}',
      '.dsp-title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600;line-height:20px}',
      '.dsp-header-actions{display:flex;align-items:center;gap:4px}',
      '.dsp-header-btn{width:30px;height:30px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:transparent;border:none;border-radius:8px;justify-content:center;align-items:center;padding:0;display:inline-flex;font-size:14px}',
      '.dsp-header-btn:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '.dsp-settings{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1,var(--dsw-alias-bg-base));flex:none;align-items:center;gap:8px;padding:10px 14px;display:flex;flex-wrap:wrap}',
      '.dsp-settings-label{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}',
      '.dsp-settings-input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:200px;height:30px;color:var(--dsw-alias-label-primary);font:inherit;font-size:13px;border-radius:8px;flex:1;padding:0 10px;outline:none}',
      '.dsp-settings-input:focus{border-color:var(--dsw-alias-brand-primary)}',
      '.dsp-btn{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);font:inherit;font-size:12px;cursor:pointer;background:transparent;border-radius:8px;height:30px;padding:0 12px}',
      ...
      '.dsp-list{box-sizing:border-box;border-right:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1,var(--dsw-alias-bg-base));width:232px;flex:none;padding:8px;overflow-y:auto}',
      ...
      '.dsp-item{box-sizing:border-box;border:none;background:transparent;width:100%;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;border-radius:8px;padding:8px 10px;margin-bottom:2px;display:block;font-family:inherit;font-size:13px;line-height:18px}',
      ...
      '.dsp-reader{position:relative;flex:1;min-width:0;padding:28px 32px;overflow-y:auto}',
      // 文档工具条（固定面板头部下方，不随文档滚动，绝不遮挡标题）
      '.dsp-toolbar{position:relative;flex:none;z-index:8;background:var(--dsw-alias-bg-base);border-bottom:1px solid var(--dsw-alias-border-l1);padding:6px 16px}',
      '.dsp-toolbar-row{display:flex;align-items:center;gap:8px;flex-wrap:nowrap}',
      '.dsp-toolbar-spacer{flex:1}',
      '.dsp-toolbar-btn{border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:12px;cursor:pointer;border-radius:7px;height:28px;padding:0 10px;display:inline-flex;align-items:center;gap:4px}',
    ...
    // 面板实时宽度（供拖拽读取，无需 useRef）
    let livePanelWidth = 760
    let dragState = null
    // 标题元素引用：大纲点击跳转用
    const headingEls = {}
    ...
    function DocsPanel() {
      ...
      const [panelW, setPanelW] = React.useState(760)
      ...
      function onResizeDown(ev) {
        dragState = { startX: ev.clientX, startWidth: livePanelWidth }
        try {
      ...
      function onResizeMove(ev) {
        ...
        const next = Math.max(420, Math.min(1400, dragState.startWidth + (dragState.startX - ev.clientX)))
        livePanelWidth = next
        setPanelW(next)
      }
      ...
      return React.createElement('div', null,
        React.createElement('button', {
          type: 'button',
          className: 'dsp-hdr-btn' + (open ? ' dsp-hdr-btn-active' : ''),
          'aria-expanded': open,
          'aria-label': '我的文档',
          title: '我的文档',
        ...
        open ? React.createElement('div', null,
          ...
          React.createElement('section', {
            ...
            role: 'dialog',
            'aria-label': '我的文档',
            style: { width: panelW + 'px', maxWidth: '94vw' },
          },
            React.createElement('div', {
              className: 'dsp-resizer' + (dragState !== null ? ' dsp-resizing' : ''),
              title: '拖拽调整宽度',
              'aria-hidden': true,
            ...
            React.createElement('header', { className: 'dsp-header' },
              React.createElement('span', { className: 'dsp-title' }, '我的文档'),
    ...
    slots.inject('conversation.session.header.utilities', function () {
      return slots.register(
        { name: 'conversation.session.header.utilities', id: 'docs-panel-entry', label: '我的文档', order: 50 },
        DocsPanel
      )
    })
